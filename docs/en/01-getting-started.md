...
This page takes you from opening Thansa for the first time to Thansa being ready to talk: creating an admin account, choosing the AI provider that acts as the "brain", picking a model, and checking system status on the Settings page.

## What this covers

Thansa OS is an AI operating layer that runs on your own machine or VPS. The brain is whichever AI provider you wire in: Claude Code, ChatGPT (through Codex), Antigravity CLI, OpenRouter, OpenAI API, Anthropic API, Google Gemini, Groq or Ollama Cloud. The first-run wizard **preselects** Claude Code because that is the recommended choice, but Thansa is not locked to any provider.

Before it is usable, Thansa needs three things:
...
| **💬 ChatGPT (subscription)** | "Sign in with ChatGPT Plus/Pro (through Codex) → still reaches every Thansa MCP." | Sign in to ChatGPT on the Models page |
| **🌐 OpenRouter** | "Many cheap models in one place, still with full Thansa MCP + skills + brain file access. Only needs an API key - no sign-in." | Paste an OpenRouter API key (now or later on Models) |

Any of the three gives you a fully capable Thansa: all of them reach the connection store (MCP), read and write files in the brain, run skills, queue work and create loops. The only difference is **running shell commands**, which only the CLI engines (Claude Code, Codex, Antigravity CLI) can do. The wizard shows three cards for brevity; the **Models** page also has OpenAI API, Google Gemini, Anthropic API, Groq and Ollama, and you can change at any time.
...
Press **Start using Thansa →** to save and enter the app. The wizard also sets a default model for the provider you chose: `sonnet` for Claude Code, `gpt-5.5` for ChatGPT, `openai/gpt-4o-mini` for OpenRouter. Change it on the Models page whenever you like.
...
4. Thansa shows a link. Open it (in a new tab) and sign in at claude.ai.
5. If the page shows a code afterwards, paste it into the **paste code (if any)** box and press **Send code**. Some flows need no code; Thansa re-checks status every 3 seconds and updates the card.
...
The **↻ Re-check** button on the card reloads the sign-in status at any time. **Disconnect** signs Claude out of Thansa.

The Thansa Claude engine runs through the **Claude Agent SDK**, but the SDK still calls the `claude` CLI on the machine, so the machine running Thansa must have `claude` installed for sign-in and native MCP to work.
...
Background work runs on API providers too, not just Claude. The real difference is elsewhere: Claude Code and Codex read and write files directly in the brain and can run shell commands, while API models read and write through the Thansa vault tools and cannot run shell commands - which suits reading, summarising and note-writing.
...
- On a narrow screen (under 860px, i.e. a phone), Thansa enters light mode automatically: the graph stops even with the switch on. The app still opens on the Thansa screen as it does on a desktop - that screen already has the chat box, it simply does not draw the brain canvas.
...
It is safe: it only moves when the destination does not exist, it never overwrites, and running it repeatedly is harmless (for example moving `Javis/agents` to `agents`, `Memory` to `memory`). Afterwards Thansa reports what it moved, or "Nothing to move (already normalised)".

**The AI image provenance card** decides whether images Thansa generates carry a provenance mark (Content Credentials, the C2PA standard). The corner label reads "Keeping" or "Stripping".
...
If you boot the machine and `localhost:7777` reports **ERR_CONNECTION_REFUSED**, reopen this page. Thansa checks three causes and writes the cause straight under the card, because none of the three leaves an error anywhere:
...
**⬆ Update now** only appears when Thansa can update itself in place. Thansa does not guess from the provider name: it **actually probes** for a listening Watchtower container.
...
Press **⬆ Update now** and Thansa asks to confirm ("Update Thansa to the latest version? The app will restart; on failure the system will try to roll back."). Once you agree, a progress bar lights up through six steps:

**Prepare → Download code → Install dependencies → Restart → Health check → Done**

The running step shows ⏳, finished steps show ✅. During the run the status line reads "⏳ Updating… do not close this page." If the local source has local modifications, Thansa reports "📦 Local changes stashed in git." (stashed, not deleted). When it finishes you get "✅ Update complete. Reloading…" and the app reloads after about 1.5 seconds.

### When the new build is broken: rolling back

Thansa keeps a way back and will not strand you on a broken build:

- **Automatic rollback:** if the new build fails the health check, Thansa returns to the old one. The progress bar shows "↩ New build failed, rolling back…", then "↩ New build failed, **rolled back automatically**."
- **Manual rollback on Docker:** if the version still has not changed after a while, Thansa reports "⚠ The new build has not come up - it may have failed." and shows a **How to roll back on Docker** block with `docker compose pull && docker compose up -d`, plus a hint to pin `ghcr.io/xahoapro/thansa-os:<old-version>` and Redeploy.
...
|---|---|---|
| **Start using Thansa →** | Wizard | Saves the workspace, account and provider, then enters the app |
...
| **Disconnect** | Models | Signs the provider out of Thansa |
...
| **Enable auto-start** / **Disable auto-start** | Settings, Start with Windows | Runs Thansa in the background at Windows sign-in |
