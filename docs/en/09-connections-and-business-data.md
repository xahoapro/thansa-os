...
- **Composio** (App store group): one connection opens many apps (Gmail, Notion, Sheets, GitHub, Linear, Slack...). In Composio **For You → Connect my agent**, copy the `ck_*` consumer key for an MCP client and enter it in Thansa. Thansa uses `https://connect.composio.dev/mcp` with the `x-consumer-api-key` header; a Platform Project key (`ak_*`) or a Platform MCP server belongs to a different flow. Connect apps under **For You → Connect Apps**. If the consumer key changes, use **Reconnect** on the Thansa Composio card to update it, then **Test**; reconnecting Calendar alone does not update Thansa's stored key. **Permission note**: every action of every app runs through ONE shared Composio tool, so Thansa checks each inner action by its Composio name (LIST, GET, FETCH, SEARCH... count as reads). Read only (the default) can read data from the apps you connected and list connected accounts; anything not clearly a read is blocked. To let Thansa edit, send or delete, raise it to **Full power**, and at that point Thansa can take ANY action on the apps you connected.
