...
Trang này hướng dẫn bạn từ lúc mở Thansa lần đầu tiên cho tới khi Thansa sẵn sàng trò chuyện: tạo tài khoản admin, chọn nhà cung cấp AI làm "bộ não", chọn model, và kiểm tra trạng thái hệ thống ở trang Cài đặt.

## Tính năng này là gì

Thansa OS là một lớp điều hành AI chạy trên máy hoặc VPS của bạn. Bộ não là nhà cung cấp AI mà bạn đấu vào: Claude Code, ChatGPT (qua Codex), Antigravity CLI, OpenRouter, OpenAI API, Anthropic API, Google Gemini, Groq hoặc Ollama Cloud. Bộ cài đặt lần đầu **chọn sẵn** Claude Code vì đó là lựa chọn được khuyên dùng, chứ Thansa không bị khoá vào một nhà cung cấp nào.

Trước khi dùng được, Thansa cần 3 thứ:
...
| **💬 ChatGPT (gói subscription)** | "Đăng nhập ChatGPT Plus/Pro (qua Codex) → vẫn dùng được MCP của Thansa." | Đăng nhập ChatGPT ở trang Models |
| **🌐 OpenRouter** | "Nhiều model giá rẻ một chỗ, vẫn đủ MCP Thansa + skill + đọc/ghi file brain. Chỉ cần API key - không cần đăng nhập." | Dán API key OpenRouter (dán ngay hoặc để sau ở Models) |

Chọn cái nào cũng ra một Thansa đủ chức năng: cả ba đều gọi được kho Kết nối (MCP), đọc/ghi file trong brain, chạy skill, giao việc và tạo loop. Khác biệt duy nhất là **chạy lệnh máy**, thứ chỉ các engine CLI (Claude Code, Codex, Antigravity CLI) làm được. Wizard chỉ hiện 3 thẻ cho gọn; vào trang **Models** còn Antigravity CLI, OpenAI API, Google Gemini, Anthropic API, Groq và Ollama, đổi lúc nào cũng được.
...
Bấm **Bắt đầu dùng Thansa →** để lưu và vào app. Wizard đồng thời đặt luôn một model mặc định cho nhà cung cấp bạn chọn: `sonnet` cho Claude Code, `gpt-5.5` cho ChatGPT, `openai/gpt-4o-mini` cho OpenRouter. Đổi lại ở trang Models bất cứ lúc nào.
...
4. Thansa hiện một đường link. Bấm mở link đó (mở trong tab mới) để đăng nhập tại claude.ai.
5. Nếu sau khi đăng nhập trang hiện một mã code, dán mã đó vào ô **dán code (nếu có)** rồi bấm **Gửi code**. Một số luồng không cần dán code, Thansa tự dò lại trạng thái mỗi 3 giây và cập nhật thẻ.
...
Nút **↻ Kiểm tra lại** trên thẻ dùng để nạp lại trạng thái đăng nhập bất cứ lúc nào. Nút **Ngắt** dùng để đăng xuất Claude khỏi Thansa.

Engine Claude của Thansa chạy qua **Claude Agent SDK**, nhưng SDK vẫn gọi tới `claude` CLI trên máy, nên máy chạy Thansa phải cài sẵn `claude` thì đăng nhập và MCP native mới hoạt động.
...
Việc nền chạy được bằng cả nhà cung cấp API, không riêng Claude. Khác biệt thật nằm ở chỗ khác: Claude Code và Codex đọc/ghi file trực tiếp trong brain và chạy được lệnh máy, còn các model API đọc/ghi qua công cụ vault của Thansa và không chạy lệnh máy, nên hợp với việc đọc, tổng hợp và ghi ghi chú.
...
- Nếu màn hình hẹp (dưới 860px, tức điện thoại), Thansa tự vào chế độ nhẹ: đồ thị dừng chạy dù công tắc đang bật. App vẫn mở ở màn Thansa như trên máy tính - màn đó đã có sẵn ô chat, chỉ khác là không vẽ khoang não.
...
Thao tác này an toàn: chỉ di chuyển khi thư mục đích chưa có, không ghi đè, chạy lại nhiều lần cũng vô hại (ví dụ chuyển `Javis/agents` sang `agents`, `Memory` sang `memory`). Sau khi chạy, Thansa báo đã di chuyển gì hoặc "Không có gì cần di chuyển (đã chuẩn)".

**Thẻ Dấu nguồn gốc ảnh AI** quyết định ảnh Thansa tạo ra có mang dấu nguồn gốc (Content Credentials, chuẩn C2PA) hay không. Nhãn góc phải là "Đang giữ" hoặc "Đang gỡ".
...
Mở máy lên mà `localhost:7777` báo **ERR_CONNECTION_REFUSED** thì mở lại trang này. Thansa tự kiểm ba nguyên nhân và ghi thẳng nguyên nhân dưới thẻ, vì cả ba đều **không để lại lỗi ở đâu**:
...
Nút **⬆ Cập nhật ngay** chỉ hiện khi Thansa tự cập nhật tại chỗ được. Thansa không đoán theo tên nhà cung cấp: nó **dò thật** xem có container Watchtower đang lắng nghe không.
...
Bấm **⬆ Cập nhật ngay**, Thansa hỏi xác nhận ("Cập nhật Thansa lên bản mới nhất? App sẽ tự khởi động lại; nếu lỗi hệ thống sẽ thử quay về bản cũ."). Đồng ý xong, một thanh tiến trình hiện ra và sáng dần qua 6 bước:

**Chuẩn bị → Tải code → Cài thư viện → Khởi động lại → Kiểm tra sức khoẻ → Xong**

Bước đang chạy có dấu ⏳, bước đã qua có ✅. Trong lúc chạy, dòng trạng thái ghi "⏳ Đang cập nhật… đừng tắt trang." Nếu mã nguồn trên máy có sửa đổi cục bộ, Thansa báo "📦 Sửa đổi cục bộ đã được cất vào git stash." (cất đi chứ không xoá). Xong xuôi thì hiện "✅ Đã cập nhật xong. Đang tải lại trang…" và app tự tải lại sau khoảng 1,5 giây.

### Khi bản mới hỏng: lùi bản

Thansa có sẵn đường lùi, không bỏ bạn kẹt ở bản lỗi:

- **Tự lùi:** nếu bản mới không qua được bước Kiểm tra sức khoẻ, Thansa tự quay về bản cũ. Thanh tiến trình hiện "↩ Bản mới lỗi, đang tự quay về bản cũ…", xong thì báo "↩ Bản mới lỗi, đã **tự quay về bản cũ**."
- **Lùi tay trên Docker:** nếu sau một lúc phiên bản vẫn chưa đổi, Thansa báo "⚠ Bản mới chưa lên sau một lúc - có thể lỗi." rồi hiện khối **Cách lùi bản Docker** với lệnh `docker compose pull && docker compose up -d`, kèm gợi ý pin image `ghcr.io/xahoapro/thansa-os:<phiên-bản-cũ>` rồi Redeploy.
...
|---|---|---|
| **Bắt đầu dùng Thansa →** | Wizard | Lưu workspace, tài khoản, nhà cung cấp rồi vào app |
...
| **Ngắt** | Models | Đăng xuất nhà cung cấp khỏi Thansa |
...
| **Giữ dấu** / **Gỡ dấu** | Cài đặt, thẻ Dấu nguồn gốc ảnh AI | Bật/tắt dấu C2PA trên ảnh Thansa tạo |
| **Bật tự khởi động** / **Tắt tự khởi động** | Cài đặt, nhóm Khởi động cùng Windows | Cho Thansa chạy nền khi đăng nhập Windows |
