...
- **Composio** (nhóm Kho ứng dụng): một kết nối mở ra nhiều app (Gmail, Notion, Sheets, GitHub, Linear, Slack...). Trong Composio **For You → Connect my agent**, lấy consumer key `ck_*` dành cho MCP client rồi dán vào Thansa. Thansa dùng `https://connect.composio.dev/mcp` với header `x-consumer-api-key`; Project API key `ak_*` và MCP server tạo trong Platform thuộc luồng khác. Kết nối app ở **For You → Connect Apps**. Nếu đã đổi key, bấm **Kết nối lại** trên thẻ Composio trong Thansa để cập nhật key, rồi **Kiểm tra**; kết nối lại riêng Calendar không đổi key Thansa đã lưu. **Lưu ý về quyền**: mọi hành động của mọi app đều chạy qua MỘT tool chung của Composio, nên Thansa soi từng lệnh bên trong theo tên lệnh (LIST, GET, FETCH, SEARCH... là đọc). Mức Chỉ đọc (mặc định) đọc được dữ liệu các app đã nối và xem tài khoản đã nối; lệnh nào không chắc là đọc thì bị chặn. Muốn Thansa sửa, gửi hoặc xoá thì nâng **Toàn quyền**, và khi đó Thansa làm được MỌI hành động trên các app bạn đã nối.
...
- **Nguồn chập chờn kiểu "lên được 1 đơn, đơn thứ 2 là rớt kết nối"** (hay gặp với Pancake POS): đã sửa ở **0.52.8**. Mỗi nguồn chỉ có một phiên, và mỗi lượt chat Thansa lại hỏi lại danh sách tool trên chính phiên đó. Việc hỏi lại xếp hàng sau cái đơn đang chạy, chờ quá 20 giây thì bản cũ **giết cả phiên** - tức là giết luôn cái đơn đang lên dở, rồi lượt sau nguồn biến mất khỏi hộp công cụ nên Thansa báo "chưa đấu POS". Nay nguồn đang chạy tool được để yên, và vẫn giữ nguyên danh sách tool của lần hỏi trước. Trang Kết nối cũng thôi báo đỏ oan lúc nguồn đang bận.
