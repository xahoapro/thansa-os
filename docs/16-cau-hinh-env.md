...
| `JAVIS_GROK_BIN` | Đường dẫn thẳng tới binary `grok` (Grok Build CLI) | (trống, Thansa tự tìm trong PATH và `~/.local/bin`) | Chỉ khi máy cài nó ở chỗ lạ. |
| `GROK_DISABLE_AUTOUPDATER` | Tắt bộ tự cập nhật của Grok Build CLI. Thansa TỰ đặt `1` cho mọi lượt nó chạy `grok` | `1` (Thansa đặt, nhưng tôn trọng giá trị bạn đặt sẵn) | Đặt `0` nếu muốn CLI tự nâng cấp. Không nên: Thansa chạy nó headless, bộ cập nhật xen vào giữa lượt là in thêm chữ vào luồng kết quả hoặc ghi vào chỗ chỉ đọc. Nâng cấp tay bằng `grok update`. |
| `JAVIS_GROK_TIMEOUT` | Trần thời gian một lượt chạy của Grok Build CLI, tính bằng giây | `900` | Việc nền dài bị cắt ngang thì tăng. Trần này không chỉ là phòng xa: nếu bản CLI trên máy không khai cờ `--permission-mode` thì Thansa không truyền nó, và một lượt headless dừng lại hỏi duyệt sẽ treo im tới vô tận - đây là thứ duy nhất gỡ được ca đó. |
| `JAVIS_MAX_TOOL_ROUNDS` | Số vòng gọi tool tối đa cho MỘT lượt trả lời của các engine dùng API key (OpenRouter, OpenAI, Anthropic, Gemini, Groq, Ollama). Bị kẹp trong khoảng 1 đến 120 | `30` | Từ 0.47.1 hiếm khi phải đụng: phanh chính chống model kẹt vòng lặp là phát hiện "gọi lại cùng tool với cùng tham số" (nhắc ở vòng lặp thứ 3, dừng ở vòng thứ 5), còn trần này chỉ là lưới đỡ xa. Việc rất dài mà vẫn chạm trần thì tăng lên rồi khởi động lại Thansa. Claude Code và Codex KHÔNG chịu trần này - chúng tự quản vòng lặp của mình. |
...
Ở ví dụ thứ hai, vì `JAVIS_HOST=0.0.0.0` (public) nên Thansa tự bật bắt buộc đăng nhập, và vì đã có `JAVIS_ADMIN_PASSWORD` nên bạn đăng nhập luôn bằng tài khoản đó, không có lúc nào server trống admin.
